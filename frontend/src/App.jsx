...
import { Bot, Send, Mic, Sun, Moon, Sparkles } from "lucide-react";
...
export default function App() {
  ...
  // UUID fallback (in case crypto.randomUUID is not available)
  const makeId = () =>
  ...
  const sendMessage = async () => {
    ...
    try {
      // NOTE: ensure VITE_API_BASE_URL is set in your .env (e.g. VITE_API_BASE_URL=https://example.com)
      ...
      setChat((prev) => [
        ...prev,
        {
          id: makeId(),
          sender: "bot",
          text: `⚠ Error fetching reply: ${serverMsg}`,
        },
      ]);
    } finally {
      setLoading(false);
          ...
          <div className="flex h-16 items-center justify-between">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                className="grid h-10 w-10 place-content-center rounded-xl border border-white/10 bg-white/10 hover:bg-white/15"
        ...
        <div className="sticky bottom-4 z-40 mx-auto w-full max-w-3xl">
          <div className="mx-2 rounded-2xl border border-white/15 bg-white/10 backdrop-blur-xl shadow-[0_12px_60px_-15px_rgba(0,0,0,0.6)]">
            <div className="flex items-center gap-2 p-2 sm:p-2.5">
              ...
              <textarea
                placeholder="Message Nova… (Shift+Enter newline, Enter to send)"
                value={message}
                onChange={(e) => setMessage(e.target.value)}
                onKeyDown={handleKeyDown}
                disabled={loading}
                className="flex-1 resize-none h-11 bg-transparent placeholder:text-slate-300/70 text-[15px] text-slate-100 outline-none px-2 sm:px-3 py-3"
